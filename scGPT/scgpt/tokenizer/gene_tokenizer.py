...
try:
    ...
    _TORCHTEXT_OK = False
    # Pure-Python Vocab drop-in (torchtext ABI 비호환 시 사용)
    class Vocab:
        def __init__(self, vocab_or_stoi=None):
            if vocab_or_stoi is None or (hasattr(vocab_or_stoi, '__len__') and len(vocab_or_stoi) == 0 and not isinstance(vocab_or_stoi, dict)):
                self._stoi: Dict[str, int] = {}
                self._itos: List = []
            elif isinstance(vocab_or_stoi, Vocab):
                self._stoi = dict(vocab_or_stoi._stoi)
                self._itos = list(vocab_or_stoi._itos)
                self._default_index = getattr(vocab_or_stoi, '_default_index', None)
                return
            elif isinstance(vocab_or_stoi, dict):
                self._stoi = dict(vocab_or_stoi)
                max_idx = max(vocab_or_stoi.values()) if vocab_or_stoi else -1
                self._itos = [None] * (max_idx + 1)
                for t, i in vocab_or_stoi.items():
                    self._itos[i] = t
            else:
                self._stoi = {}
                self._itos = []
        ...
        def insert_token(self, token: str, index: int) -> None:
            ...
            if index >= len(self._itos):
                self._itos.extend([None] * (index - len(self._itos) + 1))
        ...
        def lookup_indices(self, tokens) -> List[int]:
            return [self[t] for t in tokens]

        def __call__(self, tokens):
            return self.lookup_indices(tokens)

    class _MockTorchVocab:
        @staticmethod
        def vocab(ordered_dict, min_freq=1):
            v = Vocab()
            idx = 0
            for token, count in ordered_dict.items():
                if count >= min_freq:
                    v.insert_token(token, idx)
                    idx += 1
            return v

    torch_vocab = _MockTorchVocab()
