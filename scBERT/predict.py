...
parser.add_argument("--output_path", type=str, default='./predicted_output.h5ad', help='Path to save output h5ad.')
...
novel_indices = []
with torch.no_grad():
    for index in tqdm(range(batch_size), desc= "Predicting cells"):
...
adata.obs["predicted_celltype"] = pred_list

adata.write(args.output_path)

print(f"Saved prediction result to: {args.output_path}")
