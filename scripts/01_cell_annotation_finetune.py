...
출력:  checkpoint/CellAnnotation/  (ckpt)
       results/01_cell_annotation_finetune/metrics.json
"""
...
def prepare_adata():
    ...
    # SCrna (CellFM-torch) requires: celltype, feat, batch_id, train
    adata.obs["celltype"] = adata.obs["celltype"].astype(str)
    codes = pd.Categorical(adata.obs["celltype"]).codes
    adata.obs["celltype_id"] = codes
    adata.obs["feat"]        = codes           # numeric label for CE loss
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        logger.info(f"Epoch {epoch+1}: train_loss={total_loss/len(train_loader):.4f}  test_acc={test_acc:.4f}")
